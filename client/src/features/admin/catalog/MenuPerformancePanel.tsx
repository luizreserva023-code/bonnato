...
import { calculatePeriodVariation, rankProductsBySales } from "../../../../../shared/analyticsMetrics";
...
export function MenuPerformancePanel({
  ...
  const productDetailQuery = trpc.analytics.productDetail.useQuery(
    { storeId, productId: selectedProduct?.id ?? 0, period },
    { enabled: Boolean(selectedProduct?.id), staleTime: 30_000 },
  );
  ...
  useEffect(() => {
    if (!focusProductId || !menuQuery.data?.products?.length) return;
    const product = menuQuery.data.products.find((item) => item.id === focusProductId);
    if (product) setSelectedProduct(product);
  }, [focusProductId, menuQuery.data?.products]);
  ...
  const ranked = useMemo(
    () => rankProductsBySales(menuQuery.data?.products ?? [], ranking),
        ...
        {menuQuery.isLoading || overviewQuery.isLoading ? (
          ...
          <div className="grid gap-3 md:grid-cols-5">
            {menuQuery.data.funnel.map((stage, index) => {
              ...
              const variation = calculatePeriodVariation(stage.value, previous);
              return (
          ...
          {selectedProduct && (
            <>
              ...
              {productDetailQuery.isLoading ? (
                <div className="rounded-[14px] border border-[var(--admin-border)] p-6 text-center text-sm text-[var(--admin-text-secondary)]">
                ...
                <>
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    {[
                      ["Visualizou", productDetailQuery.data?.product.views ?? selectedProduct.views],
                      ["Carrinho", productDetailQuery.data?.product.cartSessions ?? selectedProduct.cartSessions],
                      ["Checkout", productDetailQuery.data?.product.checkoutSessions ?? 0],
                      ["Comprou", productDetailQuery.data?.product.buyerSessions ?? selectedProduct.buyerSessions],
                    ].map(([label, value]) => (
                      <div key={String(label)} className="rounded-[14px] bg-[var(--admin-surface-alt)] p-4">
                        <p className="text-xs text-[var(--admin-text-secondary)]">{String(label)}</p>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div className="rounded-[14px] border border-[var(--admin-border)] p-4"><p className="text-xs text-[var(--admin-text-secondary)]">Visualização → compra</p><p className="mt-2 text-xl font-semibold">{selectedProduct.views ? formatPct(selectedProduct.conversionRate) : "—"}</p></div>
                    <div className="rounded-[14px] border border-[var(--admin-border)] p-4"><p className="text-xs text-[var(--admin-text-secondary)]">Visualização → carrinho</p><p className="mt-2 text-xl font-semibold">{selectedProduct.views ? formatPct(selectedProduct.cartRate) : "—"}</p></div>
                    <div className="rounded-[14px] border border-[var(--admin-border)] p-4"><p className="text-xs text-[var(--admin-text-secondary)]">Visualização → checkout</p><p className="mt-2 text-xl font-semibold">{productDetailQuery.data?.product.views ? formatPct(productDetailQuery.data.product.checkoutRate) : "—"}</p></div>
                    <div className="rounded-[14px] border border-[var(--admin-border)] p-4"><p className="text-xs text-[var(--admin-text-secondary)]">Checkout → compra</p><p className="mt-2 text-xl font-semibold">{productDetailQuery.data?.product.checkoutSessions ? formatPct(productDetailQuery.data.product.checkoutConversionRate) : "—"}</p></div>
                  </div>

                  <div className="rounded-[14px] border border-[var(--admin-border)] p-4">
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <p className="text-sm font-semibold text-[var(--admin-text-primary)]">Evolução no período</p>
                        <p className="text-xs text-[var(--admin-text-secondary)]">Sessões e pedidos por dia, no horário de Brasília.</p>
                      </div>
                      {productDetailQuery.data?.categoryRank.position && (
                        <AdminPill tone="brand">
                          {productDetailQuery.data.categoryRank.position}º de {productDetailQuery.data.categoryRank.total} na categoria
                        </AdminPill>
                      )}
                    </div>
                    {productDetailQuery.data?.series.length ? (
                      <div className="h-[240px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={productDetailQuery.data.series} margin={{ top: 8, right: 8, left: -22, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#ECEDEF" vertical={false} />
                            <XAxis dataKey="date" tick={{ fontSize: 10, fill: "#979AA3" }} axisLine={false} tickLine={false} />
                            <YAxis tick={{ fontSize: 10, fill: "#979AA3" }} axisLine={false} tickLine={false} allowDecimals={false} />
                            <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #E7E8EC", boxShadow: "0 8px 30px rgba(16,24,40,.08)" }} />
                            <Line type="monotone" dataKey="views" name="Visualizações" stroke="#631014" strokeWidth={2} dot={false} />
                            <Line type="monotone" dataKey="carts" name="Carrinhos" stroke="#D97706" strokeWidth={2} dot={false} />
                            <Line type="monotone" dataKey="checkouts" name="Checkouts" stroke="#3578E5" strokeWidth={2} dot={false} />
                            <Line type="monotone" dataKey="orders" name="Pedidos" stroke="#16A36A" strokeWidth={2} dot={false} />
                          </LineChart>
                        </ResponsiveContainer>
                      </div>
                    ) : (
                      <AdminEmptyState title="Sem série temporal suficiente" description="O gráfico será preenchido conforme novas sessões e pedidos forem registrados." />
                    )}
                  </div>
                </>
              )}
              <p className="text-xs leading-5 text-[var(--admin-text-secondary)]">
                Conversão usa sessões associadas ao produto. Não usamos quantidade vendida dividida por pageviews brutos.
              </p>
