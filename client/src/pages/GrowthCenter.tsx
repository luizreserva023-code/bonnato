...
export default function GrowthCenter() {
  ...
  const intelligence = trpc.platform.intelligence.dashboard.useQuery({storeId},{enabled:storeId>0});
  ...
  const generate = trpc.platform.intelligence.generateSuggestions.useMutation({onSuccess:async()=>{await intelligence.refetch();toast.success("Diagnóstico atualizado.");},onError:(error)=>toast.error(error.message)});
  const updateSuggestion = trpc.platform.intelligence.updateSuggestion.useMutation({onSuccess:async()=>{await intelligence.refetch();toast.success("Recomendação atualizada.");},onError:(error)=>toast.error(error.message)});
